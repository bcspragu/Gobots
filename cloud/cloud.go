...
// This is a hacked apart version of  go4.org/cloud/cloudlaunch, which helps
// binaries run themselves on The Cloud, copying themselves to GCE. This
// version is slightly different, and will build binaries for the architecture
...
var (
	projectID = flag.String("project_id", "", "The project ID to upload to")
)

func main() {
	flag.Parse()
	if *projectID == "" {
		log.Fatalln("Project ID not given")
	}
	var launchConfig = &cloudlaunch.Config{
		Name:         "Gobot",
		BinaryBucket: "gobot-resource",
